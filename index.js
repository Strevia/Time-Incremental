...
unitsMax = {
    ...
    days: [365, 'years'],
    months: [12, 'years'],
    decades: [10, 'centuries'],
    centuries: [10, 'millenium']
};
    ...
    if (!unit.includes('upgrades')) {
        ...
        if (time[unit].current >= time[unit].cost) {
            ...
            if (unit === 'seconds') {
                ...
                if (time.years.current >= time.cosmodecade.cost) {
                    time.cosmodecade.current++;
                    time.years.current -= time.cosmodecade.cost;
                    time.cosmodecade.cost *= 10;
...
function showUnitBlock(unit) {
    query('.' + unit + '.block').classList.add('shown')
};
...
function gameLogic() {
    time.seconds.income += (time.minutes.generators * 2 / time.tick) * ((Math.log(time.seconds.total) * time.upgrades.multiplier.bought) || 1);
    ...
    units.forEach((unit, index) => {
        ...
        if (index + 1 < units.length) {
            time[unit].generators += (time[units[index + 1]].generators * 2 / time.tick) * (Math.pow(1.5, time.months.current));
        }
